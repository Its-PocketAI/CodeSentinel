...
<table>
  <tr>
    ...
    <td width="50%">
      <img src="./docs/images/mobile-mode.png" alt="CodeSentinel 手机模式" width="36%" />
    </td>
  </tr>
  <tr>
    <td valign="top">
...
引导脚本本身仍从 `main` 拉取，但默认安装的是最新稳定 Git tag；如果仓库里还没有稳定 tag，则回退到 `main`。
...
- `CODESENTINEL_VERSION`（默认：`latest`，表示最新稳定 tag，如 `v0.0.1`）
- `CODESENTINEL_BRANCH`（可选分支覆盖；设置后改为分支模式，不再走 tag 模式）
...
- 停止生产服务
- 默认拉取最新稳定 tag，并切换到对应版本
- 如果远端还没有稳定 tag，则自动回退到分支更新模式
- 执行 `pnpm install --frozen-lockfile`
- 重启生产服务

可选环境变量：
- `CODESENTINEL_UPDATE_REMOTE`（默认 `origin`）
- `CODESENTINEL_UPDATE_TAG`（默认 `latest`；可指定为 `v0.0.1`）
- `CODESENTINEL_UPDATE_MODE`（默认 `tag`；设为 `branch` 时改为持续跟踪分支）
- `CODESENTINEL_UPDATE_BRANCH`（分支模式下使用，默认当前分支，回退 `main`）
