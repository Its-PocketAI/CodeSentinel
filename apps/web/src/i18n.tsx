...
const EN: Record<string, string> = {
  ...
  "对话助手：适合提问、规划和解释，不是终端": "Chat assistant: good for questions, planning, and explanations; not a terminal",
  "交互式 CLI": "Interactive CLI",
  "安全终端": "Secure Terminal",
  "OpenAI Codex CLI：适合改代码、执行任务、连续协作": "OpenAI Codex CLI: good for code changes, task execution, and iterative collaboration",
  "Claude Code CLI": "Claude Code CLI",
  "Claude Code CLI：适合代码理解、修改和多轮执行": "Claude Code CLI: good for understanding code, editing, and multi-step execution",
  "OpenCode CLI": "OpenCode CLI",
  "OpenCode CLI：在终端里直接驱动 OpenCode": "OpenCode CLI: drive OpenCode directly inside the terminal",
  "Gemini CLI：把输入直接发给 Gemini 命令行代理": "Gemini CLI: send input directly to the Gemini command-line agent",
  "Kimi CLI：把输入直接发给 Kimi 命令行代理": "Kimi CLI: send input directly to the Kimi command-line agent",
  "Qwen Code CLI：把输入直接发给 Qwen 命令行代理": "Qwen Code CLI: send input directly to the Qwen command-line agent",
  "Cursor 命令行模式": "Cursor CLI mode",
  "Cursor 命令行代理：在终端里以 agent / plan / ask 运行": "Cursor command-line agent: run in terminal with agent / plan / ask modes",
  "命令行": "Terminal",
  "安全终端：执行受限 shell 命令，不是 AI 对话": "Secure terminal: run restricted shell commands, not AI chat",
  "安全终端：执行受限 shell 命令，比如 ls / git / pnpm，不是 AI 对话":
    "Secure terminal: run restricted shell commands such as ls / git / pnpm; it is not AI chat.",
  "安全受限命令行": "Restricted terminal",
  "交互式安全终端": "Interactive secure shell",
  "安全命令": "Secure command",
  "无会话": "No session",
  "恢复中": "Restoring",
  "创建中": "Creating",
  "就绪": "Ready",
  "命令完成": "Command complete",
  "已退出": "Exited",
  "工作目录已就绪，点击“新建”开始。": "Working directory ready. Click “New” to start.",
  "请先选择工作目录，然后创建会话。": "Select a working directory first, then create a session.",
  "正在恢复上次会话。": "Restoring the previous session.",
  "正在创建新会话。": "Creating a new session.",
  "交互式安全终端已连接。": "Interactive secure shell connected.",
  "安全命令已就绪，可连续输入命令。": "Secure command is ready. You can run commands continuously.",
  "{tool} 已连接。": "{tool} connected.",
  "命令已完成，可直接继续输入下一条。": "Command complete. You can enter the next command directly.",
  "命令已完成（退出码 {code}），可直接继续输入下一条。":
    "Command complete (exit code {code}). You can enter the next command directly.",
  "会话已退出，点击“新建”重新进入。": "Session exited. Click “New” to enter again.",
  "会话已退出（退出码 {code}），点击“新建”重新进入。":
    "Session exited (exit code {code}). Click “New” to enter again.",
  "保存的会话已失效，请重新创建。": "Saved session expired. Create a new one.",
  "新建会话": "New session",
  "终端状态：{state}；能力：{capability}；{detail}":
    "Terminal status: {state}; capability: {capability}; {detail}",
  "提醒": "Alerts",
  "回放": "Replay",
  "历史": "History",
  "当前会话": "Current Session",
  "历史记录": "History",
  "回放与产物": "Replay & Artifacts",
  "终端内容优先": "Terminal First",
  "当前工具：{tool}；提醒在终端顶部，历史单独查看。":
    "Current tool: {tool}; alerts stay at the top of the terminal, while history is separate.",
  "未提供": "Not provided",
  "无": "None",
  "Cursor Chat：对话式助手，用来提问、规划和解释，不直接作为终端执行命令。":
    "Cursor Chat: a conversational assistant for asking, planning, and explaining; it does not execute like a terminal.",
  "安全终端：用于执行受限 shell 命令；适合 ls、git、pnpm 这类操作，不是 AI 聊天。":
    "Secure terminal: for restricted shell commands such as ls, git, and pnpm; it is not AI chat.",
  "Cursor CLI：把你的输入直接发给 Cursor 命令行代理；当前子模式为 {mode}。":
    "Cursor CLI: sends your input directly to the Cursor command-line agent; current submode is {mode}.",
  "上方切换的是工具（如 Qwen Code / Codex CLI / 安全终端）；提醒在终端顶部，历史在单独标签，不会改变当前 AI。":
    "The top row switches tools (such as Qwen Code / Codex CLI / Secure Terminal); alerts stay at the top of the terminal, history stays in its own tab, and neither changes the active AI.",
  "{tool}：{desc}": "{tool}: {desc}",
  ...
  "Codex CLI 官方键位": "Codex CLI Keymap",
  "OpenAI Codex 官方文档/源码": "OpenAI Codex docs/source",
  "完整键位以官方链接最新版本为准；本表整理自官方 TUI 文档与源码中的快捷键定义。":
    "See the official link for the latest complete keymap. This table summarizes shortcuts from the official TUI docs and source code.",
  "打开命令列表（commands）": "Open command list (commands)",
  "Shell 命令输入": "Shell command input",
  "插入换行": "Insert newline",
  "队列发送消息（queue message）": "Queue message",
  "文件路径补全": "File path completion",
  "粘贴图片到输入区": "Paste image into input",
  "在外部编辑器编辑输入": "Edit input in external editor",
  "编辑上一条消息": "Edit previous message",
  "查看 transcript / 任务列表": "View transcript / task list",
  "切换协作模式（启用时）": "Switch collaboration mode (when enabled)",
  "编辑最近排队消息（默认终端）": "Edit latest queued message (default terminals)",
  "编辑最近排队消息（Apple Terminal/Warp/VSCode 回退）": "Edit latest queued message (Apple Terminal/Warp/VS Code fallback)",
  "切换上一个/下一个 Agent（多 Agent 时）": "Switch previous/next agent (multi-agent sessions)",
  "中断任务；双击可退出": "Interrupt task; double-press to exit",
  "输入为空时退出（双击确认）": "Exit on empty input (double-press to confirm)",
  "Claude Code 官方键位": "Claude Code Keymap",
  "完整键位请以官方链接页面为准。": "See the official page for the latest complete keymap.",
  "历史反向搜索": "Reverse history search",
  "后台执行 Bash 任务": "Run Bash task in background",
  "进入 Bash 模式": "Enter Bash mode",
  "自动补全/接受建议": "Autocomplete / accept suggestion",
  "退出 Bash 前缀输入（空输入时）": "Exit Bash prefix input (when empty)",
  "切换任务列表视图": "Toggle task list view",
  "Gemini CLI 官方键位": "Gemini CLI Keymap",
  "关闭对话框/建议": "Close dialog / suggestions",
  "取消请求并清空输入；双击退出": "Cancel request and clear input; double-press to exit",
  "清屏": "Clear screen",
  "切换调试控制台": "Toggle debug console",
  "切换工具说明显示": "Toggle tool description display",
  "切换自动审批（YOLO）": "Toggle auto-approval (YOLO)",
  "接受自动补全": "Accept autocomplete",
  "粘贴文本/图片（图片会插入引用）": "Paste text/image (images insert references)",
  "Kimi CLI 官方键位": "Kimi CLI Keymap",
  "Kimi CLI 交互与输入": "Kimi CLI interaction/input",
  "切换 Agent / Shell 模式": "Switch Agent / Shell mode",
  "发送消息": "Send message",
  "粘贴文本/图片附件": "Paste text/image attachments",
  "路径补全并引用文件": "Complete path and attach file",
  "结构化问题导航与确认/跳过": "Structured prompt navigation and confirm/skip",
  "Qwen Code 官方键位": "Qwen Code Keymap",
  "Qwen Code 键盘快捷键": "Qwen Code keyboard shortcuts",
  "反向搜索历史": "Reverse history search",
  "接受建议": "Accept suggestion",
  "在外部编辑器打开输入": "Open input in external editor",
  "粘贴文本/图片并插入引用": "Paste text/image and insert reference",
  "OpenCode 官方键位": "OpenCode Keymap",
  "完整键位请以官方链接页面和 tui.json 默认键位为准。":
    "See the official page and default tui.json for the latest complete keymap.",
  "Leader(默认 Ctrl+X)": "Leader (default Ctrl+X)",
  "大多数快捷键前缀": "Prefix for most shortcuts",
  "退出应用": "Exit application",
  "命令列表": "Command palette",
  "Agent 循环切换": "Cycle through agents",
  "粘贴输入": "Paste input",
  "终端挂起": "Suspend terminal",
  ...
  "搜索模式": "Search mode",
  "搜索范围：{path}": "Search scope: {path}",
  "项目根目录": "Project root",
  "当前目录": "Current folder",
  "自定义目录": "Custom folder",
  "目录路径": "Folder path",
  "请先选择搜索目录": "Please choose a search directory first",
  "输入目录路径，例如 codesentinal/apps/web": "Enter a folder path, e.g. codesentinal/apps/web",
  "搜索文件名，例如 App.tsx": "Search file names, e.g. App.tsx",
  "搜索文件内容，例如 useEffect": "Search file content, e.g. useEffect",
  "可按文件名或文件内容搜索当前项目。": "Search the current project by file name or file content.",
  "可按文件名或文件内容搜索当前范围。": "Search the current scope by file name or file content.",
  "暂无搜索结果": "No search results",
  "共 {count} 个结果": "{count} results",
  "结果已截断，仅显示前 {count} 项": "Results truncated, showing the first {count}",
  "第 {line} 行": "Line {line}",
  "新建文件夹": "New folder",
  "新建文件": "New file",
  "无根目录": "No root folder",
