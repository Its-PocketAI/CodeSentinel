...
export function createAuthToken(token: string, now: number, expiresAt: number): void {
  const db = getDb();
  db.prepare(`
    INSERT INTO auth_tokens (token, created_at, last_used_at, expires_at)
    VALUES (?, ?, ?, ?)
  `).run(token, now, now, expiresAt);
}

export function getAuthToken(token: string): AuthToken | null {
  const db = getDb();
  const row = db.prepare(`
    SELECT token, created_at as createdAt, last_used_at as lastUsedAt, expires_at as expiresAt
    FROM auth_tokens
    WHERE token = ?
...
export function touchAuthToken(token: string, now: number, expiresAt: number): void {
  const db = getDb();
  db.prepare(`
    UPDATE auth_tokens
    SET last_used_at = ?, expires_at = ?
    WHERE token = ?
  `).run(now, expiresAt, token);
}
