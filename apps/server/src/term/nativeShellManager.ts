...
export class NativeShellManager {
  ...
  open(cwd: string, cols = 120, rows = 30, runAs?: RunAsUser | null) {
    ...
    const s: NativeSession = { id, cwd, cols, rows, lineBuf: "", queue: [], running: false, runAs };
  ...
  private async runLine(s: NativeSession, rawLine: string) {
    ...
    const { bin, argsPrefix } = shellForLine(s.runAs ?? null);
